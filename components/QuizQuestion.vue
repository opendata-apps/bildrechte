    ...
    <div class="rounded-2xl p-4 sm:p-6 text-text-500 dark:text-text-50">
        <div class="mb-6">
            <div class="h-48 sm:h-64 flex items-center justify-center mb-4">
                <NuxtImg v-if="question.image" :src="getImageSrc(question)" :format="'webp'" :alt="question.alt"
                    class="max-w-full max-h-full object-contain rounded-lg shadow-sm" />
            </div>
            <button @click="toggleImageInfo"
            ...
            <div v-if="showImageInfo" class="text-xs sm:text-sm text-text-400 dark:text-text-300 mb-2">
                ...
                <p>Quelle: <a :href="question.source" target="_blank" rel="noopener noreferrer"
                        class="text-primary-500 hover:underline dark:text-primary-400 dark:hover:text-primary-300">{{
                            question.author || 'Unbekannt' }}</a></p>
                <p>Zuletzt besucht: {{ question.last_visited }}</p>
            </div>
            <p class="text-right text-xs sm:text-sm text-text-400 dark:text-text-300 mb-4" aria-live="polite">Frage {{
                currentQuestion + 1
                }} von {{ totalQuestions }}</p>
        </div>
        <h2 id="question-text" class="text-lg sm:text-xl font-semibold text-text-500 dark:text-text-50 mb-6">{{
            question.questions }}</h2>
        <div role="group" aria-labelledby="question-text" class="space-y-3 sm:space-y-4">
        ...
        <div v-if="question.help_query" class="mt-6">
            <button @click="toggleHelpInfo"
                ...
                :aria-expanded="showHelpInfo">
                Hilfe
            </button>
            <div v-if="showHelpInfo" class="mt-4 p-3 sm:p-4 bg-background-200 dark:bg-dark-accent rounded-lg">
                <p class="font-semibold text-text-500 dark:text-text-50 mb-2 text-sm sm:text-base">{{
                    question.help_query }}</p>
